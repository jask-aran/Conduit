...
- Keep one attached browser per terminal. Permit an explicit takeover that disconnects the current owner, and permit concurrent attachments to different terminals.
...
- The terminal header projects its selected tmux window title, foreground command, and active connection state with width-based omission. Ctrl/Cmd-wheel changes the xterm or Ghostty font size, persists it locally, fits the renderer, and publishes the resulting PTY geometry.
