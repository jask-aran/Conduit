...
test("an interrupted turn still reports its assistant message", async () => {
  ...
  assert.equal(published.some((event) => event.type === "message_end" && event.message?.role === "assistant"), false,
    "assistant settlement uses only the normalized generation event");
});
