...
test("legacy Pi rows gain durable identity without changing JSONL or exposing paths", async (t) => {
  ...
  assert.equal(JSON.parse(await fs.readFile(registry, "utf8")).version, 5);

  const draft = await store.create(project, { templateId: "coding", templateVersion: "1" });
  await store.update(draft.id, { backend: { ...draft.backend, opaqueSession: file }, templateVersion: "2" });
...
test("profile selection uses current names and rejects conflicting legacy fields", () => {
  ...
  assert.throws(() => profileSelection({ profileId: "host-pi", runtimeKind: "conduit_profile" }), { code: "invalid_profile" });
...
test("agent-managed Codex identity persists without Pi compatibility fields", async (t) => {
  ...
  assert.equal("piSessionFile" in restored.metadata(chat.id), false);
});
