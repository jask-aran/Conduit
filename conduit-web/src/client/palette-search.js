 ...
 * 4. Weak fuzzy hits are dropped (cmdk's defaultFilter is very permissive)
 *
 * When searching, results are a flat score-sorted list so models and dynamic
 * sources are not trapped under static command groups (cmdk does not reliably
 * reorder groups by score).
 */
...
function contiguousFuzzy(haystack, needle) {
  if (!needle) return 1;
...
export function commandMatchParts(command) {
  const label = command.label || "";
  const keywords = [command.label, ...(command.keywords || [])].filter(Boolean);
  const haystack = [
    command.searchValue,
    command.id,
    command.detail,
...
export function modelMatchParts(model) {
  const label = model.label || model.spec || "";
  const keywords = [model.label, model.provider, model.spec, model.id].filter(Boolean);
 ...
 * Each row: { kind: "command"|"model", score, command?, model?, group }
 */
export function rankPaletteResults({ commands, models, query, currentModel }) {
