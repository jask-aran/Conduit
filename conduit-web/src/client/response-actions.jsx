...
export function ResponseActions({ message, precedingUserId, partialContinue, editing, onCopy, onEdit, onRegenerate, onContinue }) {
  const [copied, setCopied] = useState(false);
  useEffect(() => { if (!copied) return undefined; const timer = setTimeout(() => setCopied(false), 1600); return () => clearTimeout(timer); }, [copied]);
  if (message.role === "user" && !String(message.id || "").startsWith("user_")) return <MessageFooter className="response-actions">
    <ButtonGroup><Action label={editing ? "Cancel editing" : "Edit from here"} onClick={() => onEdit(message)}><PencilIcon /></Action></ButtonGroup>
  </MessageFooter>;
  if (message.role === "user") return null;
  return <MessageFooter className="response-actions"><ButtonGroup>
