...
import { createVoiceDictationClient, type VoiceDictationState } from "./static-047-voice-dictation-client";
import type { AudioSignalLevel } from "./static-047-voice-audio";
import { toast } from "solid-sonner";
import { audioTransferLost, beginDictatedRange, matchesShortcut, releasesShortcut, replaceDictatedRange, shouldAutoSend, shouldReportNoSignal } from "./static-047-voice-dictation.js";
import { createVoiceWaveformController, Static047VoiceWaveform } from "./static-047-voice-waveform";
...
export function Static047Composer(props: {
  ...
  return <div class="composer-047-wrap">
    ...
    <Show when={props.chat.queue().steering.length || props.chat.queue().followUp.length}>
      <div class="composer-047-queue"><span>Queued messages</span><Button variant="ghost" size="sm" onClick={props.chat.clearQueue}>Restore to draft</Button></div>
    </Show>
    <div class="composer-047">
      <div class="composer-047-input-shell">
        <textarea
          ref={input}
          aria-label="Message Pi"
          aria-expanded={slashOpen()}
          aria-controls={slashOpen() ? "composer-047-slash-suggestions" : undefined}
      ...
      <Show when={slashOpen()}>
        <div id="composer-047-slash-suggestions" role="listbox" aria-label="Suggestions" class="composer-047-slash-suggestions">
      ...
      <div class="composer-047-actions">
        <div class="composer-047-actions-left">
          ...
          <Menu>
            <MenuTrigger class="composer-047-model-trigger" aria-label={`${selectedModel()?.label || props.models.model() || "Model"} ${props.models.effort() || "off"}`} disabled={!props.serverOnline}>
          ...
          <Show when={props.profiles.length}>
            <Menu><MenuTrigger class="composer-047-model-trigger" aria-label={`Profile ${props.activeProfile?.label || "General"}`} disabled={!props.serverOnline || props.chat.status() !== "draft"}><span>{props.activeProfile?.label || "Profile"}</span><ChevronDownIcon /></MenuTrigger>
        ...
        <div class="composer-047-actions-right">
    ...
    <Show when={dictationError()}><div class="composer-047-dictation-error" role="alert"><TriangleAlertIcon />{dictationError()}</div></Show>
    <div class="agent-activity composer-047-status" role="status" aria-live="polite">
      <Show when={dictating()}>
        <Static047VoiceWaveform class="composer-047-status-waveform" history={dictationWaveform.history} level={dictationWaveform.level} peak={dictationWaveform.peak} state={recorderMonitorState()} variant="compact" barCount={24} ariaLabel={dictationState() === "connecting" ? "Connecting microphone" : "Microphone input level"} />
      </Show>
      <span class="composer-047-status-state">
      ...
      <span class="composer-047-status-segment">{contextDetail()}</span>
      <Show when={queueCount()}><span class="composer-047-status-segment">Queue {queueCount()}</span></Show>
    </div>
  </div>;
}
