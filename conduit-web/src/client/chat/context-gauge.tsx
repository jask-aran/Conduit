...
import { Menu, MenuContent, MenuGroup, MenuLabel, MenuSeparator, MenuTrigger } from "@/components/primitives";
...
export function ContextGauge(props: { chat: ActiveChatStore; metrics?: () => readonly ContextMetricId[] }) {
  ...
  return <Menu modal={false}>
    <MenuTrigger class="chat-context-trigger" data-state={tone()} aria-label={`Context usage: ${Math.round(percent())}%`} title={`Context usage: ${Math.round(percent())}%`}>
      ...
      <span class="chat-context-percent">{Math.round(percent())}%</span>
    </MenuTrigger>
