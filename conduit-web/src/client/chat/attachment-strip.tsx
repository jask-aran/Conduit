...
// The file viewer's icons. Its icon theme lives in the workspace's lazy chunk,
// so it is fetched on the first file chip rather than carried by the composer.
const WorkspaceFileIcon = lazy(() => import("../workspace/file-type-icon").then((module) => ({ default: module.FileTypeIcon })));
const FileMark = (props: { name: string }) => <Suspense fallback={<FileIcon />}><WorkspaceFileIcon name={props.name} /></Suspense>;
  ...
  return <div ref={chip} class="attachment-chip" data-kind={isImage(props.item) ? "image" : "file"} data-status={props.item.status} data-entering={props.entering || undefined} title={failed() ? `${props.item.name}: ${props.item.error || "Upload failed"}` : props.item.name}>
    <button type="button" class="attachment-chip-body" aria-label={retryable() ? `Retry ${props.item.name}` : `Show ${props.item.name}`} onClick={() => retryable() ? props.onRetry() : props.onOpen()}>
      <span class="attachment-chip-media">
        <Show when={source} fallback={<FileMark name={props.item.name} />}><AttachmentImage src={source!} /></Show>
            ...
            <For each={props.items.map((item) => item.id)}>{(id) => {
              ...
              return <Show when={item()}>{(current) => <li class="attachment-dialog-row" data-status={current().status}>
                <span class="attachment-dialog-preview" data-kind={isImage(current()) ? "image" : "file"}>
                  <Show when={source()} fallback={<FileMark name={current().name} />}><AttachmentImage src={source()!} /></Show>
                </span>
...
export function AttachmentStrip(props: {
  ...
  // Removing everything fades the strip at once, not chip by chip as each
  // removal lands; one that fails brings the strip back with what is left.
  const [clearing, setClearing] = createSignal(false);
  createEffect(() => { if (props.items.length > 0 && !clearing()) setExit("drop"); });
  ...
  const removeAll = async () => {
    setExit("none");
    setClearing(true);
    await Promise.all(props.items.map((item) => props.onRemove(item)));
    setClearing(false);
  };
  const remove = async (chip: HTMLElement, item: UploadAttachment) => {
    ...
    const faded = await fadeOut(chip, 100);
    // The chip gives its room up now, not when the server confirms, and the
    // chips after it slide over by transform. A removal that fails puts it back.
    ...
    if (!reduced()) {
      for (const [node, left] of before) {
        ...
        if (Math.abs(dx) > 0.5) node.animate([{ transform: `translateX(${dx}px)` }, { transform: "none" }], { duration: 180, easing: "cubic-bezier(.2, .8, .2, 1)" });
      }
    }
    measure();
    if (await props.onRemove(item) === false) { chip.hidden = false; faded?.cancel(); measure(); }
  };
  ...
  return <>
    <Show when={shown.value().length}>
      <div class="attachment-strip composer-surface-material composer-card" data-composer-surface={props.surface} data-leaving={(clearing() ? "fade" : shown.leaving()) || undefined} role="group" aria-label="Attachments">
        ...
        <div class="attachment-strip-actions">
          <Button variant="ghost" size="icon-sm" aria-label="Show all attachments" title="Show all attachments" onClick={() => setOpen(true)}><Maximize2Icon /></Button>
          <Button variant="ghost" size="icon-sm" aria-label="Remove all attachments" title="Remove all" onClick={() => void removeAll()}><Trash2Icon /></Button>
        </div>
      </div>
    </Show>
    <AttachmentsDialog open={open()} onOpenChange={setOpen} items={props.items} chatId={props.chatId} onRemove={props.onRemove} onRemoveAll={() => void removeAll()} />
  </>;
}
