...
**Trace header** — two lines, live and settled. The first is `🧠 Status · time · work`: status is one verb in the foreground at weight 560, always shown -- live, a running tool by what it does and to what (`Searching “gassho houses”`, `Editing notes.md`, `Running npm test`, from the kind and subject the harness adapter states), `Running N tools` for several at once, Thinking between tools, Writing once the answer streams; settled, Done, Interrupted or Failed. Time is muted and tabular, seconds always two digits (`01s` … `59s`, then `1m 02s`), ticking from the prompt while live and prompt to last reply once settled. Work is single-word counts by kind (commands, reads, edits, searches, fetches, tools), every kind, most used first; left out when no tool ran. The second line is the latest thinking, muted and cut to one line, held open while the turn runs so nothing jumps when it arrives; a turn with no thinking is one line. Icon and chevron sit on the first line.
