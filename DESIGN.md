...
**Trace header** — one line, live and settled: `🧠 Status · time · work · thinking`. Status is one verb in the foreground at weight 560, always shown -- live Thinking, Running `<tool>` while one tool runs, Used N tools after several in a row, Writing once the answer streams; settled Done, Interrupted or Failed. Time is muted and tabular, seconds always two digits (`01s` … `59s`, then `1m 02s`), ticking from the prompt while live and prompt to last reply once settled. Work is single-word counts by the kind the harness adapter states (commands, reads, edits, searches, fetches, tools), in first-appearance order, three named then `+N`; left out when no tool ran. The thinking preview is last and the only part that is cut short.
